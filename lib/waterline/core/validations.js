...
Validator.prototype.validate = function(values, presentOnly, cb) {
  ...
  function validate(validation, cb) {
    ...
    if(hasOwnProperty(self.validations[validation], 'type') && self.validations[validation].type.indexOf('geometry') === 0) {
      // TODO: define validation for geometry values
      console.log('warning: no validation applied to type ', self.validations[validation].type);
      return cb();
    }
